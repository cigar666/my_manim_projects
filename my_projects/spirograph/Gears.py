...
class Spirograph_explain(SpirographScene):
    ...
    def construct(self):
        ...
        arrow = VGroup(Arrow(self.spirograph.outline_small.get_center(), self.spirograph.hole_group[-1].get_center(), buff=0, color=YELLOW))

        def update_arrow(arr):
            arr.remove(arr[0])
            arr.add(Arrow(self.spirograph.outline_small.get_center(), self.spirograph.hole_group[-1].get_center(), buff=0, color=YELLOW))
