...
class Show_Curve_by_func(Scene):
    ...
    def construct(self):
        ...
        if self.colorful_mode:
            ...
            self.add(curves)
            # self.play(ShowCreation(curves), run_time=30)
        else:
            self.add(curve)
            # self.play(ShowCreation(curve), run_time=30)
...
class Curve_nr2r(Show_Curve_by_func):
    ...
    def construct(self):
        ...
        t_total = lcm(lcm(self.T[0], self.T[1]), self.T[2])
        print('t_total: %.2f' % t_total)
        curve_r = ParametricFunction(function=lambda t: complex_to_R3(my_curve_rotate(t, self.r_list, w_list, self.phi_list, self.P_list)), t_min=0, t_max=t_total, color=BLUE_D, stroke_width=1.2, step_size=t_total/1000/self.precision_factor).scale(0.6, about_point=ORIGIN)

        t_total = lcm(self.T[0], self.T[1])
        print('t_total: %.2f' % t_total)
        curve_nr = ParametricFunction(function=lambda t: complex_to_R3(my_curve(t, self.r_list, w_list, self.phi_list, self.P_list)), t_min=0, t_max=t_total, color=BLUE_D, stroke_width=1.2, step_size=t_total/1000/self.precision_factor).scale(0.6, about_point=ORIGIN)
...
class Curve_Transform(Show_Curve_by_func):
    ...
    def construct(self):
        ...
        if self.rotate_or_not:
            ...
            print('t_total: %.2f' % t_total)
            curve = ParametricFunction(function=lambda t: complex_to_R3(my_curve_rotate(t, self.r_list, w_list, self.phi_list, self.P_list)), t_min=0, t_max=t_total, color=BLUE_D, stroke_width=1.2, step_size=t_total/1000/self.precision_factor).set_height(7.2).move_to(ORIGIN)
            curve.add_updater(lambda c: c.become(ParametricFunction(function=lambda t: complex_to_R3(my_curve_rotate(t, self.r_list, w_list, self.phi_list, self.P_list)), t_min=0, t_max=t_total, color=BLUE_D, stroke_width=1.2, step_size=t_total/1000/self.precision_factor).set_height(7.2).move_to(ORIGIN)))
        else:
            t_total = lcm(self.T[0], self.T[1])
            print('t_total: %.2f' % t_total)
            curve = ParametricFunction(function=lambda t: complex_to_R3(my_curve(t, self.r_list, w_list, self.phi_list, self.P_list)), t_min=0, t_max=t_total, color=BLUE_D, stroke_width=1.2, step_size=t_total/1000/self.precision_factor)#.set_height(6).move_to(ORIGIN)
            curve.add_updater(lambda c: c.become(ParametricFunction(function=lambda t: complex_to_R3(my_curve(t, self.r_list, w_list, self.phi_list, self.P_list)), t_min=0, t_max=t_total, color=BLUE_D, stroke_width=1.2, step_size=t_total/1000/self.precision_factor)))
        self.add(curve)
        self.wait()
