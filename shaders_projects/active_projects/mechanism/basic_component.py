...
class Rod(VGroup):
    ...
    def add_tip_1(self, loc):
        tip = Dot(loc, color=self.color, stroke_width=0, radius=self.tip_type_1['radius'])# .set_height(2 * self.tip_type_1['radius'])
        self.add(tip)
        return tip
    ...
    def get_point_by_alpha(self, alpha=0.5):
        return self.get_start() + alpha * (self.get_end() - self.get_start())
